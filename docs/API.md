...
| GET | `/system/rbac/permissions` `/system/rbac/roles` | `system.rbac.read` |
| POST | `/system/rbac/permissions` `/system/rbac/roles` | `system.rbac.create` |
| PUT | `/system/rbac/permissions/:id` `/system/rbac/roles/:id` | `system.rbac.update` |
| DELETE | `/system/rbac/permissions/:id` `/system/rbac/roles/:id` | `system.rbac.delete` |
| GET | `/system/users` | `system.users.read` |
| POST | `/system/users` | `system.users.create` |
| PUT | `/system/users/:id` | `system.users.update` |
...
| GET | `/system/api-tokens` `/system/api-tokens/:id` `/system/api-tokens/permission-options` | `system.api_tokens.read` |
| POST | `/system/api-tokens` | `system.api_tokens.create` |
| PUT | `/system/api-tokens/:id` | `system.api_tokens.update` |
| DELETE | `/system/api-tokens/:id` | `system.api_tokens.delete` |
...
| POST | `/ai/project-weekly-report` `/ai/project-risk-summary` `/ai/register-image-description` `/ai/task-breakdown` | `ai.read` + 对应源数据 read 权限 |
...
### GET `/system/rbac/permissions`
### POST `/system/rbac/permissions`
### PUT `/system/rbac/permissions/:id`
### DELETE `/system/rbac/permissions/:id`
### GET `/system/rbac/roles`
### POST `/system/rbac/roles`
- 请求体: `{ name, description, permissionIds: number[] }`
### PUT `/system/rbac/roles/:id`
### DELETE `/system/rbac/roles/:id`
...
- `images`: 图片数组，必须来自上传目录、`mimeType` 为 `image/*`，单张不超过 50M；每张可带 `remark`，备注不超过 500 字
- 规则: `projectId` 必须当前用户可见；`taskId` 如填写，任务必须当前用户可见且属于同项目；负责人和参与人必须是有效用户；`dueAt` 如填写必须是 RFC3339；状态默认 `open`，等级默认 `medium`

#### `images[]` 字段
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `fileName` | string | 是 | 上传接口返回的原始文件名 |
| `filePath` | string | 是 | 上传接口返回的公开路径，必须位于 `/static/uploads` |
| `relativePath` | string | 否 | 原始相对路径或文件名 |
| `fileSize` | number | 是 | 单张图片大小，不能超过 `52428800` 字节 |
| `mimeType` | string | 是 | 必须以 `image/` 开头 |
| `remark` | string | 否 | 图片备注，可手填或调用 AI 图片说明接口生成后回填，最多 500 字 |
...
### GET `/system/api-tokens`
- 权限: `system.api_tokens.read`
- Query: `page` `pageSize` `keyword` `isEnabled`
- 范围: 管理员查看全部；普通用户仅查看自己创建的 Token

### GET `/system/api-tokens/permission-options`
- 权限: `system.api_tokens.read`
- 效果: 返回可分配给 Token 的权限候选列表

### GET `/system/api-tokens/:id`
- 权限: `system.api_tokens.read`
- 范围: 管理员或 Token 创建人

### POST `/system/api-tokens`
- 权限: `system.api_tokens.create`
...
### PUT `/system/api-tokens/:id`
- 权限: `system.api_tokens.update`
- 范围: 管理员或 Token 创建人
- 请求体同创建；不会重新生成明文 Token；已撤销 Token 不可更新

### DELETE `/system/api-tokens/:id`
- 权限: `system.api_tokens.delete`
- 范围: 管理员或 Token 创建人
...
### POST `/ai/register-image-description`
- 权限: `ai.read`；新增登记项按 `projectId` 校验 `projects.read` 和项目可见范围，已有登记项按 `registerId` 校验 `registers.read` 和登记册可见范围
- 请求体: `{ projectId?, registerId?, image }`
- `image`: `{ fileName, filePath, relativePath?, fileSize, mimeType, remark? }`，必须来自上传目录且为图片文件
- 响应: `{ remark, requiresConfirmation, generatedAt }`
- 说明: AI 说明只返回草稿，不自动保存；前端回填到图片备注，用户保存登记项后才持久化
- 常见错误: `AI_NOT_CONFIGURED` 表示 AI 网关未配置；`INVALID_REGISTER_IMAGE` 表示图片路径、类型或大小不合法；`AI_IMAGE_DESCRIPTION_FAILED` 表示模型调用失败
...
- 安全边界: 除 `/ai/register-image-description` 会读取用户指定的上传图片外，AI 助理不会读取附件正文；不会读取财务字段、密码、密钥或审计敏感数据；没有源权限时不会把对应源数据纳入草稿
