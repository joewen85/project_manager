...
- 完成：按 `Improvement.md` 推进 P0-5 基础自动化规则状态变更触发器 MVP
- 完成：状态变更规则支持按项目范围、变更前状态、变更后状态匹配任务状态写事件
- 完成：状态变更事件可自动通知任务执行人与项目负责人、添加任务评论，并写入 `runSource=event` 的执行日志
- 完成：状态变更自动化已接入任务整单更新、状态更新、进度到审核、审核完成路径
- 完成：自动化规则前端页面支持触发器筛选、状态变更条件、自动评论动作配置
- 完成：补充状态变更规则条件校验/通知/评论/事件日志/重复触发集成测试，并同步 API/OpenAPI/RBAC/README/PLAN
- 完成：按 `Improvement.md` 推进 P0-5 基础自动化规则进度变更触发器 MVP
- 完成：进度变更规则支持按项目范围、变更前进度区间、变更后进度区间匹配任务进度写事件
- 完成：进度变更事件可自动通知任务执行人与项目负责人、添加任务评论，并写入 `runSource=event` 的执行日志
- 完成：进度变更自动化已接入任务整单更新、进度更新、状态/完成接口隐式补进度路径
- 完成：自动化规则前端页面支持进度变更触发器筛选、进度区间条件、自动评论动作配置
- 完成：补充进度变更规则条件校验/手动跳过/通知/评论/事件日志/重复触发集成测试，并同步 API/OpenAPI/RBAC/README/PLAN
- 完成：按 `Improvement.md` 推进 P0-5 基础自动化规则执行人变更触发器 MVP
- 完成：执行人变更规则支持按项目范围和新增/移除执行人类型匹配 `PUT /tasks/:id` 写事件
- 完成：执行人变更事件可自动通知当前任务执行人与项目负责人、添加任务评论，并写入 `runSource=event` 的执行日志
- 完成：自动化规则前端页面支持执行人变更触发器筛选、新增/移除条件和自动评论动作配置
- 完成：补充执行人变更规则条件校验/手动跳过/通知/评论/事件日志/重复触发集成测试，并同步 API/OpenAPI/RBAC/README/PLAN
- 完成：按 `Improvement.md` 推进 P0-5 基础自动化规则添加标签动作 MVP
- 完成：自动化动作新增 `addTags/tagIds`，支持逾期手动/定时规则和状态/进度/执行人事件规则追加任务缺失标签
- 完成：添加标签动作保持幂等，重复执行不重复写入 `task_tags`，执行日志按实际新增标签数记录动作数量
- 完成：自动化规则前端页面支持读取标签列表、配置添加标签动作并在规则列表展示目标标签
- 完成：补充添加标签动作条件校验/逾期手动执行/状态事件执行/重复执行集成测试，并同步 API/OpenAPI/RBAC/README/PLAN
- 完成：按 `Improvement.md` 推进 P0-5 基础自动化规则指派执行人动作 MVP
- 完成：自动化动作新增 `assignAssignees/assigneeIds`，支持逾期手动/定时规则和状态/进度/执行人事件规则追加任务缺失执行人
