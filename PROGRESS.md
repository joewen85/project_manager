...
- 完成：按 `Improvement.md` 完成 P2-3 预算、成本与合同附件治理 MVP
- 完成：项目新增预算、实际成本、预计收益、合同编号和合同附件元数据字段，并提供显式 SQL 迁移
- 完成：新增 `finance.read`、`finance.update` 字段级权限；普通 `projects.read` 用户无法看到财务字段，写入财务字段必须具备 `finance.update`
- 完成：项目详情与列表返回预算使用率和超预算状态，项目成本超过预算时向项目负责人发送站内预警
