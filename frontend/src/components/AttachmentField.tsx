...
export function AttachmentField({ inputId, value, onChange }: AttachmentFieldProps) {
  ...
  return (
    <div className="attachment-field">
      <input id={inputId} ref={fileInputRef} className="sr-only" type="file" multiple onChange={(event) => { void handleFileInputChange(event) }} disabled={uploading} />
      <input ref={folderInputRef} className="sr-only" type="file" multiple onChange={(event) => { void handleFileInputChange(event) }} disabled={uploading} />
      <div className="attachment-toolbar">
        <button type="button" className="btn secondary" onClick={() => fileInputRef.current?.click()} disabled={uploading}>上传多个文件</button>
        <button type="button" className="btn secondary" onClick={() => folderInputRef.current?.click()} disabled={uploading}>上传文件夹（自动压缩 ZIP）</button>
      </div>
      <div
      ...
      >
        {uploading ? '正在上传，请稍候...' : '将文件或文件夹拖放到这里上传（文件夹会自动压缩为 ZIP）'}
      </div>
      {value.length > 0 && (
        <div className="attachment-list">
          {value.map((item) => (
            <div key={item.filePath} className="attachment-item">
              ...
              <button type="button" className="btn secondary" onClick={() => removeAttachment(item)}>移除</button>
            </div>
          ))}
        </div>
      )}
