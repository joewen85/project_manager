...
- 外部客户/供应商门户（一次性邀请链接、对外任务进度、外部请求、外部评论、附件白名单）
- 任务更新/删除
- 任务评论 / @提及 / 活动流
- 进度列表
- 统计分析
...
- 项目基线与关键路径（计划快照、基线偏差、可见任务关键路径、甘特图关键路径高亮、关键路径逾期联动健康度）
- 风险、问题、决策登记册（项目级登记项、负责人/参与人、活动流、通知、Dashboard 健康度联动）
- 项目预算、成本与合同附件治理（财务字段脱敏、预算使用率、超预算预警、合同附件元数据）
- AI 助理（项目周报草稿、风险摘要、任务拆解建议，默认只读并保留人工确认）
- 个人工作（我的任务 / 我的创建 / 我的参与）
- 站内通知（未读计数 / 已读管理）
...
- 外部门户使用 `pmp_` 邀请 Token 访问，不创建外部账号；Token 只保存哈希、前缀和后四位，明文仅创建邀请时返回一次，公开门户只展示授权项目基础信息、`externalVisible=true` 的任务、门户来源评论和邀请白名单附件
- 外部门户提交的请求和评论会标记 `source=portal` 并记录外部姓名、邮箱、公司；内部 `requesterId`/`authorId` 使用邀请创建人兜底，以兼容现有审批、评论、通知和审计链路
- 任务工时字段必须为非负数；用户默认周容量范围为 `0-168` 小时，Dashboard 按当前用户可见任务聚合本周成员负载并标记过载
- 迭代任务范围通过独立关联表维护，不写入任务主表；普通用户只能查看自己创建或包含自己可见任务的迭代
- 项目基线快照只包含当前用户可见任务；基线列表普通用户仅查看自己创建的基线，管理员可查看全部
- 关键路径只基于当前用户可见任务和可见依赖计算；若关键路径任务逾期且未完成，项目健康度会自动标记为红色并输出原因
- 变更申请必须选择当前用户可见目标任务并填写至少一个变更项；审批通过后由 `requests.update` 用户应用到目标任务，应用会写任务活动、通知申请人和执行人变更对象
- 风险/问题/决策登记册按当前用户项目可见范围读写；关联任务必须当前用户可见且属于同项目；创建/更新会写登记项活动、审计并通知创建人、负责人、参与人和项目成员；未关闭高风险登记项和未解决问题会进入项目健康度评分
- 项目预算、实际成本、预计收益、合同编号和合同附件属于财务敏感数据；仅 `finance.read`、`finance.update` 或管理员可查看，写入必须具备 `finance.update`，成本超过预算时会向项目负责人发送站内预警，项目 CSV 仅对有财务读取权限的用户追加财务列
- AI 助理只生成草稿和建议，不直接创建或更新项目/任务；入口需要 `ai.read`，读取项目、任务、评论/活动和登记册上下文仍分别受对应 read 权限与可见范围限制，且不会读取财务字段、附件正文、密码、密钥或审计敏感数据
...
- `VITE_AI_ASSISTANT_TIMEOUT_MS`（AI 助理前端请求超时，默认 `35000`，需略大于后端 `AI_TIMEOUT_SECONDS`）
...
- 前端使用 `POST /api/v1/ai/*/stream` 的 SSE 响应展示生成进度，并在模型支持时通过 `delta` 事件逐段渲染模型输出；原有 JSON 接口继续保留，便于脚本和第三方集成。
...
VITE_AI_ASSISTANT_TIMEOUT_MS=35000
AI_PROMPT_DIR=
```
...
- 数据库迁移：`backend/migrations/`（评论/活动流、请求/模板、自动化、资源容量与工时字段、保存报表、项目预算成本与合同附件字段、外部门户字段已提供显式 SQL；本地测试仍通过 GORM AutoMigrate 初始化）
